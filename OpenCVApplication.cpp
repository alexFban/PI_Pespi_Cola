	...
	for (int i = 0; i < images->size; i++) {
		if (images->data[i].label == 0) {
			...
			if (images->data[i].generated_label == images->data[i].label) {
				...
				std::filesystem::rename(images->data[i].path, "misclassified/" + std::to_string(i) + "_misclassified_coca_cola.jpg");
			}
		}
			...
			if (images->data[i].generated_label == images->data[i].label) {
				...
				// le mut si le renumesc
				std::filesystem::rename(images->data[i].path, "misclassified/" + std::to_string(i) + "_misclassified_pepsi.jpg");
			}
		}
	}
